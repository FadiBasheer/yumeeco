...
List<TimeOfDay> getAmHours(List<TimeOfDay> times) => times
    .where((t) => t.hour < TimeOfDay.hoursPerPeriod)
    .map((e) => e.replacing(minute: 0))
    .toSet()
    .toList();

List<TimeOfDay> getPmHours(List<TimeOfDay> times) => times
    .where((t) => t.hour >= TimeOfDay.hoursPerPeriod)
    .map((e) => e.replacing(minute: 0))
    .toSet()
    .toList();

List<TimeOfDay> getMinutesForHour(TimeOfDay time, List<TimeOfDay> times) =>
    times.where((element) => element.hour == time.hour).toList();
...
extension FancyIterable on Iterable<TimeOfDay> {
  TimeOfDay get max => this.length > 0
      ? reduce((curr, next) => curr.hour > next.hour ? curr : next)
      : TimeOfDay(hour: null, minute: null);

  TimeOfDay get min => this.length > 0
      ? reduce((curr, next) => curr.hour < next.hour ? curr : next)
      : TimeOfDay(hour: null, minute: null);

  int get distance => this.max.hour != null && this.min.hour != null
      ? this.max.hour - this.min.hour
      : 0;
}
...
class _DayPeriodControl extends StatefulWidget {
  ...
  @override
  __DayPeriodControlState createState() => __DayPeriodControlState();
}

class __DayPeriodControlState extends State<_DayPeriodControl> {
  void _togglePeriod() {
    final int newHour =
        (widget.fragmentContext.selectedTime.hour + TimeOfDay.hoursPerPeriod) %
            TimeOfDay.hoursPerDay;
    final TimeOfDay newTime =
        widget.fragmentContext.selectedTime.replacing(hour: newHour);
    widget.fragmentContext.onTimeChange(newTime);
  }

  void _setAm(BuildContext context) {
    if (widget.fragmentContext.selectedTime.period == DayPeriod.am) {
      return;
    }
    switch (widget.fragmentContext.targetPlatform) {
  ...
  void _setPm(BuildContext context) {
    if (widget.fragmentContext.selectedTime.period == DayPeriod.pm) {
      return;
    }
    switch (widget.fragmentContext.targetPlatform) {
      case TargetPlatform.android:
  ...
  Widget build(BuildContext context) {
    ...
    final TextTheme headerTextTheme = widget.fragmentContext.headerTextTheme;
    final TimeOfDay selectedTime = widget.fragmentContext.selectedTime;
    final Color activeColor = widget.fragmentContext.activeColor;
    final Color inactiveColor = widget.fragmentContext.inactiveColor;
    ...
    final TextStyle pmStyle = headerTextTheme.subtitle1
        .copyWith(color: !amSelected ? activeColor : inactiveColor);
    final bool layoutPortrait = widget.orientation == Orientation.portrait;
      ...
      child: Material(
        ...
        child: InkWell(
          onTap: Feedback.wrapForTap(
              () => {_setAm(context), fadi = false}, context),
          child: Padding(
    ...
    final Widget pmButton = ConstrainedBox(
      ...
      child: Material(
        ...
        child: InkWell(
          onTap: Feedback.wrapForTap(
              () => {_setPm(context), fadi = true}, context),
          child: Padding(
            padding: layoutPortrait
...
//    } else {
  ...
  int pmMinHour(List<TimeOfDay> pmHours, List<TimeOfDay> amHours) =>
      pmHours.min.hour != null ? pmHours.min.hour : amHours.min.hour;

  int amMinHour(List<TimeOfDay> pmHours, List<TimeOfDay> amHours) =>
      amHours.min.hour != null ? amHours.min.hour : pmHours.min.hour;
...
//    int hoursPerPeriod =
//        _activeRing == _DialRing.outer && pmHours.distance != null
//            ? (pmHours.distance + 1)
//            : (amHours.distance + 1);

    if (fadi == true)
      hoursPerPeriod = (pmHours.distance + 1);
    else
      hoursPerPeriod = (amHours.distance + 1);
    int minTimeForPeriod = _activeRing == _DialRing.outer
        ? pmMinHour(pmHours, amHours)
        : amMinHour(pmHours, amHours);
...
//          ? ((fraction * (amHours.distance + 1)).round() %
//                  (amHours.distance + 1)) +
//              amHours.min.hour
//          : (((fraction * (pmHours.distance + 1)).round() %
//                  (pmHours.distance + 1)) +
//              pmHours.min.hour);

      int newHour;
      if (fadi == true) {
        newHour = (((fraction * (pmHours.distance + 1)).round() %
                    (pmHours.distance + 1)) +
                pmHours.min.hour) -
            12;
      } else {
        newHour = ((fraction * (amHours.distance + 1)).round() %
                (amHours.distance + 1)) +
            amHours.min.hour;
      }
...
//      } else {
    ...
    ///////////////////////////////////////////////////////////////////////// If I want costum minutes also ///////////////////////////////////////////
//    else {
//      List<TimeOfDay> minutes = getMinutesForHour(widget.selectedTime, times);
//      var minute = minutes.max.minute == 0
//          ? 0
//          : (fraction * (minutes.max.minute + 15)).round() %
//              (minutes.max.minute * 15);
...
//            () {
//              print('timeOfDay.houram1: ${timeOfDay.hour}');
...
//                alwaysUse24HourFormat: media.alwaysUse24HourFormat),
//            () {
//              print('timeOfDay.hour: ${timeOfDay.hour}');
//              _selectHour(timeOfDay.hour);
//            },
//          ),
//      ];
//////////////////////////////////////////////////////////////////////////////////////////// I nned to do another for for Am an change betwwwen them depending on choose
        ...
        for (final TimeOfDay timeOfDay in getPmHours(times))
          _buildTappableLabel(
            ...
            localizations.formatHour(timeOfDay,
                alwaysUse24HourFormat: media.alwaysUse24HourFormat),
            () {
              print('timeOfDay.houram: ${timeOfDay.hour}');
              _selectHour(timeOfDay.hour);
            },
          ),
      ];

  List<_TappableLabel> _build12HourOuterRingAm(TextTheme textTheme) =>
      <_TappableLabel>[
        for (final TimeOfDay timeOfDay in getAmHours(times))
          _buildTappableLabel(
            textTheme,
...
//    return <_TappableLabel>[
//      for (final TimeOfDay timeOfDay
//          in getMinutesForHour(widget.selectedTime, times))
//        _buildTappableLabel(
//          textTheme,
//          timeOfDay.minute,
...
//        } else {
        if (fadi == true) {
          ...
          selectedDialValue = widget.selectedTime.hourOfPeriod;
          primaryOuterLabels = _build12HourOuterRingAm(theme.textTheme);
          secondaryOuterLabels = _build12HourOuterRingAm(theme.accentTextTheme);
        }
        //       }
        break;
